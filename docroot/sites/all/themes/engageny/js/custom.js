  ...
  $(document).ready(function() {
    ...
    jQuery(".toggle_title").toggle(
	 ...
	 $(function(){
	   ...
	   if ( path )
		 $('.lessons-item-list a[href$="' + path + '"]').attr('class', 'selected');
	 });

	$('.subscriptionblock a').each(function() {
		var text = $(this).text();
		$(this).text(text.replace('Subscribe to: This post', 'Subscribe to: this page'));
		$(this).text(text.replace('Subscribe to: Posts by', 'Subscribe to: pages created by'));
	});

	  $('.nysedpostedfacet a').html('<img src="/sites/all/themes/engageny/images/postedbyengage.png" border="0" alt="Posted by EngageNY"/>');
    // @TODO Find a simpler way of doing this with facetapi.
    $('.nysedpostedfacet .item-list ul li').eq(0).addClass('last');
    $('.nysedpostedfacet .item-list ul li').eq(1).remove();
	//  $('.nysedpostedfacet a').css('display', '');

	$(function(){
		var isChecked = $('.nysedpostedfacet .facetapi-checkbox').is(':checked');
		if(isChecked) {
			$('.nysedpostedfacet li').html('<input class="facetapi-checkbox" checked="true" type="checkbox"><a style="" href="/video-library" rel="nofollow" class="facetapi-active facetapi-checkbox-processed">\n<img src="/sites/all/themes/engageny/images/postedbyengage.png" border="0" alt="Posted by EngageNY">\n</a>');
		}
	});

	$(function(){
		$("#edit-retain-filters").prop("checked", true);
	});

	$(function(){
		$("#edit-group-access").prop('disabled',true);
	});
        // Tooltip only Text
        $('.masterTooltip').hover(function(){
                ...
                $(this).attr('title', ''); // this line is used to remove the title attribute in IE on a hover
