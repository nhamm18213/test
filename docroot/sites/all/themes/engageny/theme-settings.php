 ...
 */
function engageny_form_system_theme_settings_alter(&$form, $form_state) {
  ...
  if (theme_get_setting('slider_display')) {
    ...
    foreach ($banners as $image_data) {
      $form['slider']['banner']['images'][$i] = array(
          ...
          'image' => _engageny_banner_form($image_data),
      );
    ...
    $form['slider']['banner']['image_upload'] = array(
     ...
     $form['#submit'][] = 'engageny_settings_submit';
  }
 ...
 * Save settings data.
 */
function engageny_settings_submit($form, &$form_state) {
  $settings = array();
  ...
  if ($file = file_save_upload('image_upload')) {
    ...
    if ($image = _engageny_save_image($file)) {
 ...
 *    Folder to check
 */
function _engageny_check_dir($dir) {
 ...
 *    FALSE on error.
 */
function _engageny_save_image($file, $banner_folder = 'public://banner/', $banner_thumb_folder = 'public://banner/thumb/') {
  // Check directory and create it (if not exist)
  _engageny_check_dir($banner_folder);
  _engageny_check_dir($banner_thumb_folder);
...
/**
 * Provvide default installation settings for engageny.
 */
function _engageny_install() {
  // Deafault data
  $file = new stdClass;
  $banners = array();
  // Source base for images

  $src_base_path = drupal_get_path('theme', 'engageny');
  ...
  foreach ($default_banners as $i => $data) {
    ...
    $banner = _engageny_save_image($file);
  ...
  // Save banner data
  engageny_set_banners($banners);

  // Flag theme is installed
  variable_set('theme_engageny_first_install', FALSE);
}
 ...
 */
function _engageny_banner_form($image_data) {
  $img_form = array();
